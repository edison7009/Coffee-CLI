// Tool Dictionary Loader — resolves localized data per tool type
//
// Each tool (claude-code, etc.) has its own dictionary under
// src/dictionaries/<tool>/. <lang>.json provides slashCommands, hotkeys,
// guides, and tips.
...
export function getToolDictionary(tool: ToolType, _lang: string = 'zh-CN'): ToolDictionary {
  ...
  if (!langData) return getGlobalFallback(_lang);
 ...
 */
export function getToolPanelConfig(_tool: ToolType): PanelConfig {
