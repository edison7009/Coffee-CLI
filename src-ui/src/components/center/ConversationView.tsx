...
import { evaluateScrollEvent, planLayoutScrollTop, SCROLL_SCROLLABLE_SLACK_PX } from '../../lib/conversation-scroll';
...
function ConversationViewImpl({
  ...
  const lastScrollGeometryRef = useRef({ scrollHeight: 0, scrollTop: 0 });
  // Set by the layout effect when a transcript commit shortened the content;
  // consumed by the next scroll event (lib/conversation-scroll: clamps).
  const transcriptShrankRef = useRef(false);
  const lastLayoutMessagesRef = useRef<ChatMessage[] | null>(null);
  ...
  useEffect(() => {
    ...
    // like a clamp and would be yanked back mid-drag). The custom scrollbar
    // reports its drags through bindAutoHideScrollbar's onDrag below.
    const noteInput = () => {
      lastScrollInputAtRef.current = Date.now();
      transcriptShrankRef.current = false;
    };
    // Keyboard scrolling reaches this scroller even when focus sits on <body>
    // (Chromium scrolls the last-clicked container), where a keydown listener
    // on the element itself never fires — listen on window instead.
    ...
    const onScroll = (event: Event) => {
      ...
      // report). A clamp is recognised by geometry, not by the input clock,
      // so one landing right after a wheel tick is still not owned. Pinned
      // positions always record; their scrollTop is unused for restore, but
      // the pinned flag itself must survive remounts.
      const scrollHeight = element.scrollHeight;
      const scrollTop = element.scrollTop;
      const previous = lastScrollGeometryRef.current;
      lastScrollGeometryRef.current = { scrollHeight, scrollTop };
      lastKnownScrollHeightRef.current = scrollHeight;
      const transcriptShrank = transcriptShrankRef.current;
      transcriptShrankRef.current = false;
      ...
      if (decision.record) {
        recordConversationScrollPosition(ownerKey, decision.pinned, scrollTop);
      }
  ...
  useEffect(() => {
    ...
    return bindAutoHideScrollbar(element, {
      slim: true,
      onDrag: () => { lastScrollInputAtRef.current = Date.now(); },
    });
  }, []);
  ...
  useLayoutEffect(() => {
    ...
    // A transcript commit that shortened the content will clamp scrollTop;
    // flag it so the resulting scroll event is not mistaken for the reader.
    // Height changes without a transcript change (row measurement) never set
    // this — those clamps follow the user's own jump toward the bottom.
    if (lastLayoutMessagesRef.current !== messages) {
      transcriptShrankRef.current = lastLayoutMessagesRef.current !== null &&
        element.scrollHeight < lastKnownScrollHeightRef.current;
      lastLayoutMessagesRef.current = messages;
    }
    ...
    if (prependAnchor) {
      ...
      // The anchor keeps the reader on the same rows; remember the shifted
      // offset directly. Its scroll event is only owned while the input
      // window is still open, and a slow page read can outlast it.
      if (element.scrollHeight > element.clientHeight + SCROLL_SCROLLABLE_SLACK_PX) {
        recordConversationScrollPosition(ownerKey, false, element.scrollTop);
      }
      return;
    }
    // Pinned views follow the bottom. Otherwise restore the remembered
    // reading position when the browser lost it: a display:none round-trip,
    // a clamp after the transcript shrank under the viewport (tool
    // rewrote/compacted its session file mid-poll), or a remount. The DOM
    // alone cannot tell that from a real scroll-to-top — the remembered
    // position can. The recent-input guard keeps a fast scrollbar fling
    // (whose scroll events lag a frame behind the drag) from being yanked back.
    const target = planLayoutScrollTop({
      pinned: pinnedRef.current,
      savedScrollTop: conversationScrollPositions.get(ownerKey)?.scrollTop ?? null,
      clientHeight: element.clientHeight,
