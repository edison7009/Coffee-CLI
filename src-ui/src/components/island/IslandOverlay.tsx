...
const HermesIcon = ({ size = 20 }: { size?: number }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
...
const OpenCodeIcon = ({ size = 20 }: { size?: number }) => (
...
export function IslandOverlay() {
  ...
  useEffect(() => {
    ...
    listen<AgentStatusPayload>('agent-status', (event) => {
      const { id, status, tool } = event.payload;
      ...
      setAgents(prev => {
        ...
        next.set(id, {
          id,
          tool: tool || existing?.tool || 'claude', // prioritize exact payload tool
          status: islandStatus,
          updatedAt: Date.now(),
        });
        return next;
