...
- Distributing a **modified fork** under any of the Marks, or under a name
  confusingly similar to them (e.g., "Coffee CLI Pro", "Coffee CLI Plus",
  "Better Coffee CLI", "VibeID Plus", "Sentinel Protocol Next").
- Using the Coffee CLI **logo, icon, or wordmark** in product marketing,
  app-store listings, paid advertising, or merchandise.
- Operating a **commercial SaaS or hosted service** that uses any of the
  Marks in its name, branding, or domain.
...
### 4. Forks: what to rename

If you fork Coffee CLI and intend to distribute the fork, you must:

1. **Change the product name** to something clearly distinct (not just a
   prefix or suffix on a Mark).
2. **Replace the logo** with your own, or remove it.
3. **Update the user-facing strings** in `src-ui/` and any installer copy.
4. **Keep the AGPL-3.0 license, NOTICE, and TRADEMARKS.md files intact**,
   and add your own copyright notice alongside (do not remove ours).

You may state factually that your fork is "based on Coffee CLI" or "a fork
of Coffee CLI" — this is nominative fair use and is permitted.
...
- 以任何标识、或与之**易混淆的近似名称**发布**修改后的 fork**
  (例如:"Coffee CLI Pro"、"Coffee CLI 中国版"、"Better Coffee CLI"、
  "VibeID Plus"、"新哨兵协议")。
- 在产品宣传、应用商店上架、付费广告或周边商品中使用 Coffee CLI 的
  **Logo、图标或文字商标**。
- 运营在名称、品牌或域名中使用任何标识的**商业 SaaS 或托管服务**。
...
若你 fork 本项目并打算分发,你必须:

1. **更改产品名称**为显著区别的名字(不能只是在原标识前后加前/后缀)。
2. **替换 Logo** 为你自己的,或移除。
3. **更新 `src-ui/` 与安装器中的用户可见字符串**。
4. **保留 AGPL-3.0、NOTICE 与 TRADEMARKS.md 文件原貌**,在其旁追加
   你的版权声明(不得删除原作者声明)。
